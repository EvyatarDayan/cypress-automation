...
let personalityLayoutId;

describe('Create personality', () => {
  before(async () => {
    const layouts = await fetchAllLayouts();
    personalityLayoutId = findLayoutId('personality-quiz-two', layouts);
  });

  it('Login', () => {
    cy
      .loginToPortal(res.automationUsers.user3.email, res.automationUsers.user3.password)
      .goto(`${Cypress.env('EDITOR_PUBLIC_URL')}/editor/new?layoutId=${personalityLayoutId}`)
  ...
  it('Create the unit', () => {
    ...
    // Add cover title
      .typeValue('[engine="personality-quiz-two"] > .main-title > .ape-text-editor-wrapper > .ta-root', `Personality cover: ${randomNumber}`)
      .waitFor(1000)
      .scrollToPosition(0, 400)
    // Add slide title
      .typeValue('[max-length="180"] > .main-title > .ape-text-editor-wrapper > .ta-root', `Personality slide: ${randomNumber}`)
    ...
    // Add result 1 text
      .typeValue(':nth-child(1) > .result-option > .ng-pristine', 'Result 1')
    // Add result 2 text
      .typeValue(':nth-child(2) > .result-option > .ng-pristine', 'Result 2')
    // Add extra result field
      .clickOn('.result-box > button')
    // Add result 3 text
      .typeValue('.result-option > .ng-pristine', 'Result 3')
...
// todo: cannot save personality unit (401)
